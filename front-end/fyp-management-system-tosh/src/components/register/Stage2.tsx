...
const Stage2 = ({ setStage }: { setStage: (stage: number) => void }) => {
  ...
  useEffect(() => {
    if (state.data) {
      const uniData = state.data.data.map((uniItem: any) => uniItem.uniname);
  ...
  const handleSubmit = (e: any) => {
    ...
    if (Object.keys(previousErrors).length === 0) {
      ...
      !staff &&
      previousErrors.typeOfStaff &&
      Object.keys(previousErrors).length === 1
    ) {
      appendAccount(values);
      incrementStage();
    }
  ...
  return (
    <motion.div
      ...
      <h2 className="lefty" style={{ margin: '0px 0 15px' }}>
        Fill in your academic information here
      </h2>
      <Grid container spacing={3}>
        {uniOptions.length > 0 ? (
          ...
          <Grid item xs={12}>
            {' '}
            <p>
              There was an error fetching the University Data, retrying 30
              seconds
            </p>{' '}
          </Grid>
        ) : (
          <Grid item xs={12}>
            {' '}
            <CircularProgress /> <p>Fetching University Datas</p>{' '}
          </Grid>
        )}
        <Grid item xs={12}>
          <TextField
          ...
          />
          {errors.matricNumber ? (
            <FormHelperText>{errors.matricNumber}</FormHelperText>
          ) : null}
        </Grid>
        <Grid item xs={12}>
          ...
          {errors.typeOfAccount ? (
            <FormHelperText>{errors.typeOfAccount}</FormHelperText>
          ) : null}
        </Grid>
        <Grid item xs={12}>
          {staff ? (
            <motion.div
              ...
              {errors.typeOfStaff ? (
                <FormHelperText>{errors.typeOfStaff}</FormHelperText>
              ) : null}
            </motion.div>
          ) : null}
        </Grid>
